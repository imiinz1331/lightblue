...
makeEmbedding :: [((Int, Int), Int)] -> IO ()
makeEmbedding relation = do
  init <-
    sample $
      MLPSpec
  ...
  -- エンティティの埋め込み
  let entitiesTensors = map (asTensor . (map (fromIntegral :: Int -> Float))) relation
      embeddedEntities = map (mlp init) entitiesTensors -- :: [Tensor]
